...
if isEyelink
    ...
    eye.fixWinSizeDva = 3; % Width and Height of square fixation window
...
for block = 1: blockNum
    ...
    abandonTrialFlag = 0;  % 0 means functional trial 1 means the trial should be abandoned
    addTrialNum = 0;
    ...
    for trial = 1:(trialNum + addTrialNum)
        if trial > trialNum
            %           addConditionMat(addTrialNum) = [flash.QuadDegree, flash.MotDirec, probe.shiftPix, flash.maxPhaseShiftPix];
            %           flash.LocSecq
            flash.LocSecq = addConditionMat(trial - trialNum,1);
            flash.MotDirecMat(trial) = addConditionMat(trial - trialNum,2);
            probe.shiftPixMat(trial) = addConditionMat(trial - trialNum,3);
            flash.maxPhaseShiftMat(trial) = addConditionMat(trial - trialNum,4);
        end
        ...
        %         Screen('Flip', window);
        [~, gazeWinStart] = Screen('Flip', window);
        if isEyelink
                ...
                if evtype == el.SAMPLE_TYPE % if a gaze sample is detected
                    ...
                    if evt.time > StimEDFtime
                        ...
                        if inFixWindow(x_gaze,y_gaze) % If gaze sample is within fixation window (see inFixWindow function below)
                            if (GetSecs - gazeWinStart)*1000 >= eye.fixateTime % If gaze duration >= minimum fixation window time
                ...
                if isEyelink
                    ...
                    if evtype == el.SAMPLE_TYPE % if a gaze sample is detected
                        ...
                        if evt.time > StimEDFtime
                            ...
                            x_gaze = evt.gx(eyeUsed+1); % +1 as we are accessing an array
                            y_gaze = evt.gy(eyeUsed+1);
                            gazeRect=[ x_gaze-9 y_gaze-9 x_gaze+10 y_gaze+10];
                            ...
                            elseif ~inFixWindow(x_gaze,y_gaze) % If gaze sample is not within fixation window
                                ...
                                sprintf('Gaze is outside fixation window during block %d  trial  %d\n',  block, trial)
                                abandonTrialFlag = 1;
                                addTrialNum = addTrialNum + 1;
                                abandonTrialMat(block,trial) = abandonTrialFlag;
                                addConditionMat(addTrialNum) = [flash.QuadDegree, flash.MotDirecMat(trial), probe.shiftPixMat(trial), flash.maxPhaseShiftPixMat(trial)];
                            end
                        end
                    end
        ...
        while respToBeMade
            ...
            if isEyelink
                if keyCode(KbName('space'))% Identify keyboard key code for space bar to end each trial later on)
...
fix = mx > eye.fixationWindow(1) &&  mx <  eye.fixationWindow(3) && ...
    my > eye.fixationWindow(2) && my < eye.fixationWindow(4) ;
end
