...
%----------------------------------------------------------------------
%       QUEST parameter for petal (fovea) and fugal (peripheral)
%----------------------------------------------------------------------
% Define initial value for probe.shiftPix
probe.shiftPixInitial_petal = 50; % Initial value

pThreshold_petal = 0.75; % Probability of correct response at threshold
beta_petal = 3.5; % Slope of the psychometric function
delta_petal = 0.01; % Lapse rate
gamma_petal = 0.5; % Guess rate
tGuess_petal = log10(probe.shiftPixInitial_petal); % Use small value to avoid log(0); % Initial threshold guess corresponding to a linear value of 20
tGuessSd_petal = 4; % Uncertainty of the guess

% Create QUEST object
q_petal = QuestCreate(tGuess_petal, tGuessSd_petal, pThreshold_petal, beta_petal, delta_petal, gamma_petal);
% q.normalizePdf = 1; % Optional: normalize the PDF
thresholdProgression_petal = nan(blockNum, trialNum);

%----------------------------------------------------------------------
%       QUEST parameter for fugal (peripheral)
%----------------------------------------------------------------------
% Define initial value for probe.shiftPix
probe.shiftPixInitial_fugal = 5; % Initial value

pThreshold_fugal = 0.75; % Probability of correct response at threshold
beta_fugal = 3.5; % Slope of the psychometric function
delta_fugal = 0.01; % Lapse rate
gamma_fugal = 0.5; % Guess rate
tGuess_fugal = log10(probe.shiftPixInitial_fugal); % Initial threshold guess corresponding to a linear value of 20
tGuessSd_fugal = 4; % Uncertainty of the guess

% Create QUEST object
q_fugal = QuestCreate(tGuess_fugal, tGuessSd_fugal, pThreshold_fugal, beta_fugal, delta_fugal, gamma_fugal);
% q.normalizePdf = 1; % Optional: normalize the PDF
thresholdProgression_fugal = nan(blockNum, trialNum);
...
for block = 1: blockNum
    ...
    while trial <=  trialNum %
        ...
        fprintf('Block %d, Trial %d: Quad = %d, Mot = %d, Shift = %d\n', block, trial, quad, motionDirec, flashLoc);
        if motionDirec == - 1
            tTest_petal = QuestQuantile(q_petal);
            probe.shiftPixCurrent_petal = 10^tTest_petal; % Convert to linear scale
            trialPetal = trialPetal + 1;
            thresholdProgression_petal(block, trialPetal) = probe.shiftPixCurrent_petal;
            fprintf('Trial %d: QUEST suggested tTest (log scale) = %f, probe.shiftPixCurrent_petal = %f\n', trial, tTest_petal, probe.shiftPixCurrent_petal);
        elseif motionDirec == 1
            tTest_fugal = QuestQuantile(q_fugal);
            probe.shiftPixCurrent_fugal = 10^tTest_fugal; % Convert to linear scale
            trialFugal = trialFugal + 1 ;
            thresholdProgression_fugal(block, trialFugal) = probe.shiftPixCurrent_fugal;
            fprintf('Trial %d: QUEST suggested tTest (log scale) = %f, probe.shiftPixCurrent_fugal = %f\n', trial, tTest_fugal, probe.shiftPixCurrent_fugal);
        end
        ...
        % --------------------------------------------------------
        if motionDirec == - 1
            probe.CenterPosX(block,trial) = flash.CenterPosX(block,trial) - phaseshiftFactorX * probe.shiftPixCurrent_petal * sind(45);
            probe.CenterPosY(block,trial) = flash.CenterPosY(block,trial) - phaseshiftFactorY * probe.shiftPixCurrent_petal * cosd(45);
        elseif motionDirec == 1
            probe.CenterPosX(block,trial) = flash.CenterPosX(block,trial) + phaseshiftFactorX * probe.shiftPixCurrent_fugal * sind(45);
            probe.CenterPosY(block,trial) = flash.CenterPosY(block,trial) + phaseshiftFactorY * probe.shiftPixCurrent_fugal * cosd(45);
        end
        ...
        fprintf('Trial %d: responseCorrect = %f\n', responseCorrect);
        % Update QUEST object based on response
        if motionDirec == -1
            tTestAdjusted_petal = log10(probe.shiftPixCurrent_petal);
            % Update the QUEST object with the response
            q_petal = QuestUpdate(q_petal, tTestAdjusted_petal, responseCorrect);
            % Extract the updated threshold from the QUEST object
            updatedThreshold_petal = QuestMean(q_petal); % Mean of the current posterior
            % Convert back to linear scale and assign it to probe.shiftPixCurrent_petal
            probe.shiftPixCurrent_petal = 10^updatedThreshold_petal;
            fprintf('Trial %d: QuestMean  = %f\n', trial, updatedThreshold_petal);
            % Print the updated value for verification
            fprintf('Trial %d: Updated Threshold Estimate (linear) = %f\n', trial, probe.shiftPixCurrent_petal);
        elseif motionDirec == 1
            tTestAdjusted_fugal = log10(probe.shiftPixCurrent_fugal);
            % Update the QUEST object with the response
            q_fugal = QuestUpdate(q_fugal, tTestAdjusted_fugal, responseCorrect);
            % Extract the updated threshold from the QUEST object
            updatedThreshold_fugal = QuestMean(q_fugal);
            % Convert back to linear scale and assign it to probe.shiftPixCurrent_fugal
            probe.shiftPixCurrent_fugal = 10^updatedThreshold_fugal;
            fprintf('Trial %d: QuestMean  = %f\n', trial, updatedThreshold_fugal);
            % Print the updated value for verification
            fprintf('Trial %d: Updated Threshold Estimate (linear) = %f\n', trial, probe.shiftPixCurrent_fugal);
        end
    ...
    if motionDirec == - 1
        finalThreshold_guess_petal = QuestMean(q_petal);  % Calculate the mean estimate from QUEST
        yline(10^finalThreshold_guess_petal, 'r--', 'DisplayName', 'Estimated Threshold', 'LineWidth', 1.5);
    elseif motionDirec == 1
        finalThreshold_guess_fugal = QuestMean(q_fugal);  % Calculate the mean estimate from QUEST
        yline(10^finalThreshold_guess_fugal, 'r--', 'DisplayName', 'Estimated Threshold', 'LineWidth', 1.5);
    end
...
% for block = 1:blockNum
%     plot(1:trialNum, thresholdProgression_petal(block, :), '-o', 'DisplayName', ['Block ' num2str(block)]);
%     hold on;
%     plot(1:trialNum, thresholdProgression_fugal(block, :), '-o', 'DisplayName', ['Block ' num2str(block)]);
%     hold on;
% end
% 
...
% legend('show');
% grid on;
% Plot the threshold estimates for each block with separate legends for petal and fugal
for block = 1:blockNum
    % Plot for petal
    plot(1:trialNum, thresholdProgression_petal(block, :), '-o', ...
         'DisplayName', ['Block ' num2str(block) ' Petal']);
    hold on;
    % Plot for fugal
    plot(1:trialNum, thresholdProgression_fugal(block, :), '-o', ...
         'DisplayName', ['Block ' num2str(block) ' Fugal']);
    hold on;
end
...
ylabel('Threshold Estimate (probe.shiftPix)');
title('Convergence of Threshold Estimate Across Trials');
legend('show'); % This will now show all the legends for each line with unique labels
grid on;


% Optional: Add a line representing the estimated final threshold
finalThreshold_guess_petal = QuestMean(q_petal);  % Calculate the mean estimate from QUEST
yline(10^finalThreshold_guess_petal, 'r--', 'DisplayName', 'Estimated Threshold for petal', 'LineWidth', 1.5);
probe.shiftPixThreshold_petal = 10^finalThreshold_guess_petal;

finalThreshold_guess_fugal = QuestMean(q_fugal);  % Calculate the mean estimate from QUEST
yline(10^finalThreshold_guess_fugal, 'b--', 'DisplayName', 'Estimated Threshold for fugal', 'LineWidth', 1.5);
probe.shiftPixThreshold_fugal = 10^finalThreshold_guess_fugal;
