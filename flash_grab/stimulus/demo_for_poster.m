...
gratDurationInSec = 10; % grating show duration in seconds
...
flash.QuadDegree = [45 315]; % % 10 pixels [45 45 45 45]     [45 135 225 315]
...
flash.maxPhaseShiftPix = [gratingCenterPix  gratingCenterPix];
...
%                 phaseShift = phaseShift + phaseSpeed;
                ...
                if phaseShift >= flash.maxPhaseShiftMat(trial) + 100 | phaseShift <= - flash.maxPhaseShiftMat(trial) - 100
...
%         probe.PosXMat(block,trial) = probe.CenterPosX(block,trial);
%         probe.PosYMat(block,trial) = probe.CenterPosY(block,trial);
